...
//                return "<var>string</var> ({$strlen($var)}) \"{$str}&nbsp;&hellip;\"</dt><dd><pre>{$html($var, 0)}</pre></dd>";
                return "\"{$str}&nbsp;&hellip;\"</dt><dd><pre>{$html($var, 0)}</pre></dd>";
            } else {
//                return "<var>string</var>({$strlen($var)}) \"{$html($var, 0)}\"";
                return "<petik>'</petik>{$html($var, 0)}<petik>'</petik>";
            }
        } elseif (is_array($var)) {
            $output = array();
            ...
            if (empty($var)) {
                ...
                foreach ($var as $key => & $val) {
                    ...
                    $key = $isSeq ? "" : "<petik>'</petik><dfn>{$html($key, 0)}</dfn><petik>'</petik> <panah>=></panah>";
...
//                    $output[] = "<dt{$class}>{$plus}{$access} <dfn>{$key}</dfn> => {$dump}";
                    $output[] = "<dt{$class}>{$plus} <petik>'</petik><dfn>{$key}</dfn><petik>'</petik> <panah>=></panah> {$dump}";
                }
                unset($objects[$hash]);
            } else {
                // Depth too great
    ...
    private static function _wrapEnd($callee, $prevCaller) {
        ...
        return "<span class='foot'>{$calleeInfo}</span></div>";
    }
